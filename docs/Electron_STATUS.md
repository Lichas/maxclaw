...
| Phase 2: 核心功能增强 | ✅ 100% | Markdown、模型切换、定时任务、技能管理、消息搜索、会话管理、@提及、快捷命令已完成 |
...
| 技能配置文件解析 | ✅ | 前端展示完整技能信息，禁用的技能不会进入 LLM 上下文 |
...
### 高优先级（核心体验）
1. **系统通知** - Gateway 消息推送至系统通知栏
2. **全局快捷键** - 快速呼出应用（如 Cmd/Ctrl+Shift+Space）

### 中优先级（功能完善）
3. **WebSocket 实时推送** - 替代轮询，降低延迟
4. **Mermaid 图表渲染** - 流程图展示
5. **文件附件功能** - 实现 project 和附件按钮的实际功能

### 低优先级（锦上添花）
6. **Cron 表达式可视化生成器** - 定时任务创建界面优化
7. **执行历史日志** - 显示详细执行日志
