...
> 最后更新：2026-02-21（已更新 i18n、深色主题、技能描述解析）
...
| Phase 2: 核心功能增强 | ✅ 95% | Markdown、模型切换、定时任务、技能管理、消息搜索、会话管理、@提及、快捷命令、i18n、深色主题已完成 |
...
### 高优先级（核心体验）
1. **Mermaid 图表渲染** - AI 生成的流程图/时序图可视化
2. **文件附件功能** - 实现 project 和附件按钮的实际功能，支持文件拖拽上传

### 中优先级（功能完善）
3. **系统通知** - Gateway 消息推送至系统通知栏
4. **WebSocket 实时推送** - 替代轮询，降低延迟
5. **模型配置编辑器** - 完整的提供商管理（添加/编辑/测试 API）

### 低优先级（锦上添花）
6. **全局快捷键** - 快速呼出应用（如 Cmd/Ctrl+Shift+Space）
7. **Cron 表达式可视化生成器** - 定时任务创建界面优化
8. **执行历史日志** - 显示详细执行日志
9. **自动更新机制** - electron-updater 配置
10. **IM Bot 配置面板** - Telegram/Discord/飞书等配置 UI
