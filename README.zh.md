...
MaxClaw 内置了一个 **六层自适应生命周期系统**，目标不是“神奇地永不出错”，而是让长会话里的错误更容易恢复、状态更容易检查、经验更容易沉淀：

| 层级 | 作用 | 关键能力 |
|------|------|----------|
| **1. 验证层** | 错误处理 | 自动分类 15+ 类错误，并给出恢复方向（鉴权、限流、上下文溢出等） |
| **2. 反思层** | 上下文管理 | 结构化上下文压缩、Token/成本洞察 |
| **3. 适应层** | 运行时调整 | 模型回退、上下文长度调整、指数退避重试 |
| **4. 持久化层** | 会话恢复 | 文件系统检查点、恢复与续跑 |
| **5. 进化层** | 模式学习 | 错误模式识别、恢复策略效果跟踪 |
| **6. 反馈层** | 用户学习 | 规则→上下文→LLM 三层反馈检测，跨会话写入 `MEMORY.md` |

**当前版本里已经接通的关键链路：**
- CLI、Gateway、cron 三个入口都会初始化生命周期系统
- 主循环里会记录会话开始/结束、API 成功/失败、工具执行结果和每轮 checkpoint
- 遇到上下文溢出类错误时，会先做结构化上下文压缩，再在同一轮里重试，而不是直接失败退出
- 生命周期统计里的 provider 归因基于当前运行时 provider 身份，而不是误用默认模型名

**示例：上下文过长时会发生什么**
```text
Error: 400 Bad Request (context too large)
↓
[验证层] 识别为 ContextOverflowError
↓
[反思层] 把较早轮次压缩成结构化摘要
↓
[适应层] 调整运行时参数，必要时切到更大上下文窗口的 fallback model
↓
[持久化层] 保存 checkpoint，便于恢复
↓
[进化层] 记录模式：“Go 重构任务中反复触发上下文溢出”
↓
[反馈层] 如果用户后续纠正，还会沉淀偏好或教训
```

该系统默认保持本地优先：checkpoint、进化统计、反馈学习都写在本地工作区。实现细节可见 `docs/agent_lifecycle.md`。
